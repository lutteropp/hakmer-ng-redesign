...
#include "../../indexing/suffix_array_construction/RadixLSDCache.hpp"
...
#define MOD_WORDSIZE(p) ((p) & 63)
	...
	// 'charCode' maps each character of the original input to an index between 0 and alphabet size.
	void packInput(const std::string& originalInput, const unum length, const uint *charCode,
			const int bitsPerChar, word *packedInput) {
		int remainBits = bitsPerWord;
		word l = 0;
		unum nWords = 0;
		for (unum i = 0; i < length; ++i) {
			word code = charCode[(unsigned char) originalInput[i]];
			...
			if (remainBits >= 0) {
				...
				// but the left shifting will get rid of those bits later anyway
//                l = GET_LAST(code, bitsInNextWord);
	...
	template<class W>
	void shiftLeft(uint *code, int bits, W *shiftedCode) {
		for (int i = 0; i < maxChar; ++i)
		shiftedCode[i] = ((W) code[i]) << bits;
	}

	// Sorts all suffixes by their first bits, thus grouping the suffixes into buckets.
	// Returns a vector (call it bStart) of bucket starting positions in the suffix array
	// (i.e. bStart[0] = 0;
	//       bStart[1] = size of first bucket;
	//       bStart[2] = bStart[1] + size of second bucket;
	//  etc.)
	vector<unum> sortByFirstBits(uint *charCode, int bitsPerChar, int bitsPerPass) {
		...
		// bucket sizes are stored from position 2 so we put a zero in front of them (at position 1) and do a prefix sum
		// to obtain bucket starting positions
		unum *bStart = &buffer[1];
		bStart[0] = 0;
		prefixSum(bStart, bStart, n);

		// Rearrange suffixes into the corresponding buckets given by their first bits.
		{
			word w = 0;
			for (unum i = length; i--;) {
				int c = originalInput[i];
		...
		// To fix this we just shift the whole array by putting a zero in front of it!
		bStart = bStart - 1;
		bStart[0] = 0;

		// bStart is now at the beginning of the buffer so we just return the buffer
		return buffer;
	}
	...
	void sortBuckets(const int bitsToSkip, const unum nBuckets, const unum *bStart,
			const int bitsPerChar, const word *input, word *buffer,
			RadixLSDCache<unum, word, unum>& sorter) {
	...
	// Sorts a set of buckets by their next word after the first bitsToSkip.
	// The start of a bucket is marked by a non-zero flag at that position in the bucketFlag.
	void sortFlagBuckets(const int bitsToSkip, const uchar *bucketFlag, unum start, unum length,
			const int bitsPerChar, const word *input, word *buffer,
			RadixLSDCache<unum, word, unum>& sorter) {
		unum bStart = start;
		unum bLen = 1;
	...
	void detectPeriods(unum *lo, unum bLen, unum D, unum &periodLength, unum &maxNbPeriods) {
		periodLength = 0;
		maxNbPeriods = 0;
	...
	bool detectAndTreatPeriods(unum bStart, unum bLen, unum D, unum D2, uchar bucketStartFlag) {
		unum period;
		unum maxNbPeriods;
		detectPeriods(sa + bStart, bLen, D, period, maxNbPeriods);

		if (maxNbPeriods * period > 2 * D)
		return treatPeriodLeaders(bStart, bLen, D, D2, bucketStartFlag, period);
		else
		return false;
	}

	bool treatPeriodLeaders(unum bStart, unum bLen, unum D, unum D2, uchar bucketStartFlag,
			unum periodLength) {
		...
		if (leaders[0])
		k += populate<false>(leaders[0], periodLength, bStart, bLen, bucketStartFlag,
				suffixBuff3P, dest, bufferP);
		...
		if (leaders[1]) {
			...
			k += populate<true>(leaders[1], periodLength, bStart, bLen, bucketStartFlag, sb, dest,
					bf);
		}
	...
	int populate(unum leaders, unum periodLength, unum bStart, unum bLen, unum bucketStartFlag,
			unum *suffixBuff, unum *dest, word *buffer) {
	...
	void handleBucketRepeatsOnly(unum bStart, unum bLen, unum D, unum D2, uchar bucketStartFlag) {
		detectAndTreatPeriods(bStart, bLen, D, D2, bucketStartFlag);
	}

	void handleBucketNoRepeats(unum bStart, unum bLen, unum D, unum D2, uchar bucketStartFlag) {
		vector<word> bucketBuffVec;
		bucketBuffVec.reserve(bLen);
	...
	void handleBucketWithRepeats(unum bStart, unum bLen, unum D, unum D2, uchar bucketStartFlag) {
			...
			for (unum i = length; i--;) {
				...
				if (bLen > 1) {
					...
					if (flag <= allowedRepeats) {
						if (copyFromNext) {
							copyDepth += 1;
							bool treatPeriods = copyDepth >= sa[bStart] - sa[bStart + 1];
							...
							if (treatPeriods) {
								...
								for (unum i = bStart + bLen; i-- > bStart;) {
									if (bucketFlag[i]) {
										if (subLen > 1) {
											handleBucketRepeatsOnly(i, subLen, copyDepth,
							...
							handleBucketWithRepeats(bStart, bLen, D, expectedSorted, flag + 1);
	...
	// Sorts the suffixes by their first bits (notice that we use bits instead of input characters).
	// This is done in two or three passes. The first pass bucket sorts by bitsPerFirstPass bits.
	// The second and third pass further subdivides the buckets obtained in the first pass, by the next word of each suffix.
	void inputBasedSort(uint *charCode, int bitsPerChar, int bitsPerFirstPass, bool doThirdPass) {
		vector<unum> bStartVec = sortByFirstBits(charCode, bitsPerChar, bitsPerFirstPass);

		bucketFlag = vector<uchar>(length + 1);
		bucketFlag[0] = bucketFlag[length] = 1;

		unum zeros = fixEndingZeros(charCode);
		bStartVec[0] += zeros; // we can now ignore the suffixes that are all zero
		...
		sortBuckets(bitsPerFirstPass, n, &bStartVec[0], bitsPerChar, input, buffer, sorter);

		if (doThirdPass) {
			sortFlagBuckets(bitsPerFirstPass + bitsPerWord, &bucketFlag[0], 0, length, bitsPerChar,
					input, buffer, sorter);
		}
	}

public:
	Radix(const std::string& input, unum n, unum kmerLength = 0) :
	originalInput(input), length(n), bucketPiggyBackBits(unumBits - bitsFor(length)), bucketPiggyBackLimit(
			((unum) 1) << bucketPiggyBackBits), bucketPiggyBackMask(
			bucketPiggyBackLimit - 1), kmerLength(kmerLength), sa(NULL) {
	}
	...
	unum* build() {
		...
		cout << "Alphabet size " << alphaSize << "(" << bitsPerChar << " bits per char)"<< endl;
		const unsigned int bitsPerFirstPass = (18 / bitsPerChar) * bitsPerChar;
		...
		inputBasedSort(charCode, bitsPerChar, bitsPerFirstPass, doExtraRadixPass);

		if (doFullSuffixSorting) {
			cout << "Doing full suffix sorting" << endl;
			int minBitsSortedInSecondPass = doExtraRadixPass ? 2 * bitsPerWord : bitsPerWord;
			int expectedSorted = (bitsPerFirstPass + minBitsSortedInSecondPass) / bitsPerChar;

			unum sentinels = 100; // add sentinels to avoid if's in getBucketWord
...
private:
	...
	// This avoids the need for a special character ($) at the end of the string.
	unum fixEndingZeros(uint *charCode) {
		unum p = 0;
		for (unum i = length - 1; (p < length) && charCode[(unsigned char) originalInput[i]] == 0; --i)
		bucketFlag[++p] = 1;
		return p;
	}
